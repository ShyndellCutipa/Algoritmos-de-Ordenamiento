/* Implementación de MergeSort */
#include<stdlib.h> 
#include<stdio.h> 
  
// fusiona dos submatrices de arr[]. 
// El primer subgrupo es arr[l..m] 
// El segundo subgrupo es arr[m+1..r] 
void merge(int arr[], int l, int m, int r) 
{ 
    int i, j, k; 
    int n1 = m - l + 1; 
    int n2 =  r - m; 
  
    /* creación de arreglos temporales */
    int L[n1], R[n2]; 
  
    /* Se copian los datos a L[] y R[] */
    for (i = 0; i < n1; i++) 
        L[i] = arr[l + i]; 
    for (j = 0; j < n2; j++) 
        R[j] = arr[m + 1+ j]; 
  
    /* Fusiona lo arregos temp.  en arr[l..r]*/
    i = 0; // Indice inicial del primer subarray
    j = 0; // Indice inicial del segundo subarray
    k = l; // Indice inicial del subarray fusionado
    while (i < n1 && j < n2) 
    { 
        if (L[i] <= R[j]) 
        { 
            arr[k] = L[i]; 
            i++; 
        } 
        else
        { 
            arr[k] = R[j]; 
            j++; 
        } 
        k++; 
    } 
  
    /* Se copian los elementos restantes de L[], si hay alguno */
    while (i < n1) 
    { 
        arr[k] = L[i]; 
        i++; 
        k++; 
    } 
  
    /* Se copian los elementos restantes de R[], si hay alguno */
    while (j < n2) 
    { 
        arr[k] = R[j]; 
        j++; 
        k++; 
    } 
} 
  
/* l es para el índice izquierdo y r es el índice derecho del subconjunto de arr para ser ordenado */
void mergeSort(int arr[], int l, int r) 
{ 
    if (l < r) 
    { 

        int m = l+(r-l)/2; 
  
        // Ordena la primera y la segunda mitad 
        mergeSort(arr, l, m); 
        mergeSort(arr, m+1, r); 
  
        merge(arr, l, m, r); 
    } 
} 

void ImprimirArray(int A[], int size) 
{ 
    int i; 
    for (i=0; i < size; i++) 
        printf("%d ", A[i]); 
    printf("\n"); 
} 
  

int main() 
{ 
    int arr[] = {12, 11, 13, 5, 6, 7}; 
    int arr_size = sizeof(arr)/sizeof(arr[0]); 

    mergeSort(arr, 0, arr_size - 1); 
  
    printf("\nArreglo ordenado: \n"); 
    ImprimirArray(arr, arr_size); 
    return 0; 
} 